...
| #173 | “`POST /v1/tickets` requires a contribution-ready citizen, derives immutable `ownerUserId` from the session, and snapshots profile contact at submission. Reject guests with `401` and incomplete/inactive citizens with `403`; keep public browse and tracking routes unauthenticated. Never accept a client-supplied owner ID.” |
