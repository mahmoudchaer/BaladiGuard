...
def test_wait_for_gsi_tolerates_transient_missing_metadata(monkeypatch):
    client = FakeDynamoClient([None, "CREATING", "ACTIVE"])
    monkeypatch.setattr("app.database.migrations.time.sleep", lambda *_args, **_kwargs: None)
...
def test_wait_for_gsi_missing_index_times_out(monkeypatch):
