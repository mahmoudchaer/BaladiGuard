...
def _contains(haystack: str | None, needle: str) -> bool:
    ...
    return needle.casefold() in haystack.casefold() or (
        bool(_compact(needle)) and _compact(needle) in _compact(haystack)
    )
...
class StaffSearchService:
    def search(self, query: str, *, principal: StaffPrincipal) -> StaffSearchResponse:
        ...
        try:
            work_orders = self._search_work_orders(normalized, principal, scanned)
        except Exception:
            partial_failures.append("work_orders")

        try:
            workers, teams = self._search_workforce(normalized, principal)
        except Exception:
        ...
        return StaffSearchResponse(
            ...
            ticketsTruncated=len(tickets) > MAX_RESULTS_PER_TYPE,
            workersTruncated=len(workers) > MAX_RESULTS_PER_TYPE,
            teamsTruncated=len(teams) > MAX_RESULTS_PER_TYPE,
            workOrdersTruncated=len(work_orders) > MAX_RESULTS_PER_TYPE,
        ...
        if compact.startswith("wo_"):
            ...
            if loaded is not None:
                ...
                if ticket is not None and staff_can_access_ticket(principal, ticket):
                    seen.add(loaded.work_order_id)
                    hits.append(self._work_order_hit(loaded, ticket.ticket_number))
    ...
    def _search_workforce(
    ...
    ) -> tuple[list[StaffSearchWorkerHit], list[StaffSearchTeamHit]]:
        ...
        workers = [
            ...
            for item in workforce_service.list_workers(principal, municipality_id=None)
            if _contains(item.display_name, query) or _contains(item.worker_id, query)
        ]
        teams = [
            StaffSearchTeamHit(
                teamId=item.team_id,
            ...
            for item in workforce_service.list_teams(principal, municipality_id=None)
            if _contains(item.display_name, query) or _contains(item.team_id, query)
        ]
