...
class DynamoContentSafetyJobStore:
    ...
    def get(self, job_id: str) -> StoredContentSafetyJob | None:
        ...
        return StoredContentSafetyJob.model_validate(item) if item else None
    ...
    def claim_next(self, *, now: int, claim_ttl_seconds: int) -> StoredContentSafetyJob | None:
        ...
        for item in candidates:
            ...
            try:
                ...
                return StoredContentSafetyJob.model_validate(response["Attributes"])
            except ClientError as exc:
                if exc.response.get("Error", {}).get("Code") != "ConditionalCheckFailedException":
                    raise
        return None
    ...
    def recover_stale(self, *, now: int) -> list[StoredContentSafetyJob]:
        ...
        for item in self._scan(Attr("status").eq("running") & Attr("claimExpiresAt").lte(now)):
            ...
            if token and self.retry(
                ...
                recovered.append(StoredContentSafetyJob.model_validate(item))
        return recovered

    def list(self) -> list[StoredContentSafetyJob]:
        return [StoredContentSafetyJob.model_validate(i) for i in self._scan()]

    def clear(self) -> None:
        raise NotImplementedError("Use db-reset for DynamoDB-backed content-safety jobs.")
